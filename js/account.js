...
function createMinecraftAccountCard(account) {
  ...
  const mark = account.restriction ? "block" : (needsVerification ? "warning" : "bedrock");
  card.innerHTML = `<div class="minecraft-account-mark">${accountGlyph(mark, "account-item-glyph")}</div><div class="minecraft-account-copy"><strong>${accountEscape(account.playerName)}</strong><span>${accountEscape(accountMeta)}</span>${account.restriction ? `<small class="minecraft-account-reason">${accountEscape(account.restriction.reason || "Нарушение правил")}</small>` : ""}</div><div class="minecraft-account-status ${status.className}">${accountEscape(status.label)}</div>`;
  return card;
}
...
function createActivityItem(event) {
  ...
  item.innerHTML = `<div class="account-activity-mark">${accountGlyph(event.critical ? "warning" : "pulse", "account-item-glyph")}</div><div><strong>${accountEscape(event.message || "Событие профиля")}</strong><span>${accountEscape(parts.join(" · "))}</span></div>`;
  return item;
}
...
function renderDecisions() {
  ...
  decisions.forEach((decision) => {
    ...
    card.innerHTML = `<div class="account-decision-mark">${accountGlyph("balance", "account-item-glyph")}</div><div class="account-decision-copy"><span>${accountEscape(decision.label)}</span><strong>${accountEscape(decision.restriction.reason)}</strong><small>${accountEscape(decision.restriction.decisionCode)} · ${accountEscape(formatRestrictionExpiry(decision.restriction.expiresAt))}</small>${appeal ? `<em>${accountEscape(conversationStatusLabel(appeal.status))} · попытка ${appeal.attemptNumber}</em>` : ""}</div><button class="main-btn secondary account-small-action" type="button" aria-controls="accountAppealComposer" aria-expanded="false" ${disabled ? "disabled" : ""}>${accountEscape(actionLabel)}</button>`;
...
function renderSessions() {
  ...
  accountUi.sessions.forEach((session, index) => {
    ...
    row.innerHTML = `<div class="account-session-icon">${accountGlyph(session.current ? "shield" : "sessions", "account-item-glyph")}</div><div><strong>${session.current ? "Текущий сеанс" : `Сеанс ${index + 1}`}</strong><span>Активность: ${accountEscape(formatAccountDate(session.lastSeenAt, true))} · создан ${accountEscape(formatAccountDate(session.createdAt, true))}</span></div>${session.current ? `<span class="account-session-current">Текущий</span>` : `<button class="main-btn secondary account-small-action" type="button">Завершить</button>`}`;
